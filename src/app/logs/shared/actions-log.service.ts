...
export class ActionsLogService {
  ...
  searchAccessLog(params: NzTableQueryParams, search: ActionLog, paginate: boolean): Observable<ActionLog[]> {
    ...
    // Search params
    if (search) {
      if (search.user.username) queryParams += '&username=' + search.user.username;

      if (search.endpoint) queryParams += '&endpoint=' + search.endpoint;

      if (search.action) queryParams += '&action=' + search.action;
    ...
    return this.http.get<ActionLog[]>(url).pipe(catchError(this.handleError()));
  }
