...
export class RoleService {
  ...
  getRole(id: number): Observable<Role> {
    return this.http.get<Role>(`${this.baseUrl}auth/roles/${id}`).pipe(
      map((response: Role) => {
        const role = new Role();
  ...
  searchRoles(params: NzTableQueryParams, search: string, paginate: boolean): Observable<Role[]> {
    ...
    return this.http.get<Role[]>(url).pipe(catchError(this.handleError()));
  }

  createRole(role: Role): Observable<Role> {
    return this.http.post<Role>(`${this.baseUrl}auth/roles`, role).pipe(catchError(this.handleError()));
  }

  updateRole(role: Role): Observable<Role> {
    return this.http.put<Role>(`${this.baseUrl}auth/roles/${role.id}`, role).pipe(catchError(this.handleError()));
  }

  deleteRole(id: number): Observable<any> {
    return this.http.delete<any>(`${this.baseUrl}auth/roles/${id}`).pipe(catchError(this.handleError()));
  }
